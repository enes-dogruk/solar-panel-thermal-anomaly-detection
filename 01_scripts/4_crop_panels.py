#!/usr/bin/env python3
"""
ADIM 4 (video1): PANELLERI KIRP  -- ORIJINAL SURUM (geri alindi)

NOT: "kenar duzeltmesi" (ortalama-renk dolgu + yumusatilmis maske)
denendi, ama sistem-seviyesi testte (9_end_to_end_inference.py) genel
performansi ciddi sekilde dusurdu (ikili eslesme %95.1 -> %85.3, tam
eslesme %77-82 -> %57.3). Yerel bir sorunu (kadraj kenarindaki ucgen
sekilli panel 1'in Grad-CAM'de gosterdigi kararsizlik) hedefleyen
degisiklik, TUM veri setine uygulandigi icin genele yayilan bir
performans kaybina yol acti. Bu yuzden orijinal (siyah maske) yonteme
geri donuldu. Detay: staj raporu, Bolum 3.5 / 4.

Manuel sablondaki poligonlarla tum karelerden panel kirpar.
Poligon disi pikseller maskelenir (cati/komsu panel sizmaz).
Ayrica her panel icin termal istatistik CSV'si uretir.
"""
...
CROP_DIR   = Path("4_panel_crops")
STATS_CSV  = Path("4_panel_stats.csv")
...
print(f"{len(panels)} panel sablonu, {len(frames)} kare")
